...
![cdefensesidekick](https://github.com/cdefensesecurity/cdefensesidekick/raw/master/imgs/cdefensesidekick_color.png)

![release](https://flat.badgen.net/github/release/cdefensesecurity/cdefensesidekick/latest?color=green) ![last commit](https://flat.badgen.net/github/last-commit/cdefensesecurity/cdefensesidekick) ![licence](https://flat.badgen.net/badge/license/MIT/blue) ![docker pulls](https://flat.badgen.net/docker/pulls/cdefensesecurity/cdefensesidekick?icon=docker)

## Description

A simple daemon for connecting [`cdefense`](https://github.com/cdefensesecurity/cdefense) to your ecossytem. It takes a `cdefense`'s events and
forward them to different outputs in a fan-out way.

It works as a single endpoint for as many as you want `cdefense` instances :

![cdefense_with_cdefensesidekick](https://github.com/cdefensesecurity/cdefensesidekick/raw/master/imgs/cdefense_with_cdefensesidekick.png)

## Outputs

`cdefensesidekick` manages a large variety of outputs with different purposes.
...
- [**StatsD**](https://github.com/statsd/statsd) (for monitoring of `cdefensesidekick`)
- [**DogStatsD**](https://docs.datadoghq.com/developers/dogstatsd/?tab=go) (for monitoring of `cdefensesidekick`)
- [**Prometheus**](https://prometheus.io/) (for both events and monitoring of `cdefensesidekick`)
...
- **Webhook**
- [**WebUI**](https://github.com/cdefensesecurity/cdefensesidekick-ui) (a Web UI for displaying latest events in real time)

### Other
- [**Policy Report**](https://github.com/kubernetes-sigs/wg-policy-prototypes/tree/master/policy-report/cdefense-adapter)

## Adding `cdefensesecurity` repository

Prior to install the chart, add the `cdefensesecurity` charts repository:

```bash
helm repo add cdefensesecurity https://cdefensesecurity.github.io/charts
helm repo update
```

## Installing the Chart

### Install cdefense + cdefensesidekick + cdefensesidekick-ui

To install the chart with the release name `cdefensesidekick` run:

```bash
helm install cdefensesidekick cdefensesecurity/cdefensesidekick --set webui.enabled=true
```

### With Helm chart of cdefense

`cdefense`, `cdefensesidekick` and `cdefensesidekick-ui` can be installed together in one command. All values to configure `cdefensesidekick` will have to be
prefixed with `cdefensesidekick.`.

```bash
helm install cdefense cdefensesecurity/cdefense --set cdefensesidekick.enabled=true --set cdefensesidekick.webui.enabled=true
```

After a few seconds, cdefensesidekick should be running.
...
To uninstall the `cdefensesidekick` deployment:

```bash
helm uninstall cdefensesidekick
```
...
The following table lists the main configurable parameters of the cdefensesidekick chart and their default values. See `values.yaml` for full list.
...
| config.alertmanager.endpoint | string | `"/api/v1/alerts"` | alertmanager endpoint on which cdefensesidekick posts alerts, choice is: `"/api/v1/alerts" or "/api/v2/alerts" , default is "/api/v1/alerts"` |
...
| config.aws.cloudwatchlogs.logstream | string | `""` | AWS CloudWatch Logs Stream name, if empty, cdefensesidekick will try to create a log stream |
...
| config.aws.rolearn | string | `""` | AWS IAM role ARN for cdefensesidekick service account to associate with (optionnal if you use EC2 Instance Profile) |
...
| config.aws.sns.rawjson | bool | `false` | Send RawJSON from `cdefense` or parse it to AWS SNS |
...
| config.customfields | string | `""` | a list of escaped comma separated custom fields to add to cdefense events, syntax is "key:value\,key:value" |
...
| config.dogstatsd.namespace | string | `"cdefensesidekick."` | A prefix for all metrics |
...
| config.elasticsearch.index | string | `"cdefense"` | Elasticsearch index |
...
| config.influxdb.database | string | `"cdefense"` | Influxdb database |
...
| config.mqtt.topic | string | `"cdefense/events"` | Topic for messages |
...
| config.policyreport.kubeconfig | string | `"~/.kube/config"` | Kubeconfig file to use (only if cdefensesidekick is running outside the cluster) |
...
| config.spyderbat.source | string | `"cdefensesidekick"` | Spyderbat source ID, max 32 characters |
...
| config.statsd.namespace | string | `"cdefensesidekick."` | A prefix for all metrics |
...
| config.templatedfields | string | `""` | a list of escaped comma separated Go templated fields to add to cdefense events, syntax is "key:template\,key:template" |
...
| config.timescaledb.hypertablename | string | `"cdefense_events"` | Hypertable to store data events (default: cdefense_events) See TimescaleDB setup for more info  |
...
| config.wavefront.metricname | string | `"cdefense.alert"` | Metric to be created in Wavefront. Defaults to cdefense.alert |
...
| config.zincsearch.index | string | `"cdefense"` | index |
...
| image.repository | string | `"cdefensesecurity/cdefensesidekick"` | The image repository to pull from |
...
| ingress.hosts | list | `[{"host":"cdefensesidekick.local","paths":[{"path":"/"}]}]` | Ingress hosts |
...
| resources | object | `{}` | The resources for cdefensesdekick pods |
...
| webui.enabled | bool | `false` | enable cdefensesidekick-UI |
...
| webui.image.repository | string | `"cdefensesecurity/cdefensesidekick-ui"` | The web UI image repository to pull from |
...
| webui.ingress.hosts | list | `[{"host":"cdefensesidekick-ui.local","paths":[{"path":"/"}]}]` | Web UI ingress hosts configuration |
...
| webui.redis.service.port | int | `6379` | The web UI Redis service port dor the cdefensesidekick-ui |
...
| webui.service.port | int | `2802` | The web UI service port dor the cdefensesidekick-ui |
