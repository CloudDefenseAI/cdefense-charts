# Configuration values for CloudDefense chart
...
| collectors.kubernetes.apiAuth | string | `"/var/run/secrets/kubernetes.io/serviceaccount/token"` | Provide the authentication method CloudDefense should use to connect to the Kubernetes API. |
| collectors.kubernetes.apiUrl | string | `"https://$(KUBERNETES_SERVICE_HOST)"` |  |
| collectors.kubernetes.enableNodeFilter | bool | `true` | If true, only the current node (on which CloudDefense is running) will be considered when requesting metadata of pods to the API server. Disabling this option may have a performance penalty on large clusters. |
| collectors.kubernetes.enabled | bool | `true` | Enable Kubernetes meta data collection via a connection to the Kubernetes API server. When this option is disabled, CloudDefense falls back to the container annotations to grap the meta data. In such a case, only the ID, name, namespace, labels of the pod will be available. |
| containerSecurityContext | object | `{}` | Set securityContext for the CloudDefense container.For more info see the "clouddefense.securityContext" helper in "pod-template.tpl" |
...
| controller.deployment.replicas | int | `1` | Number of replicas when installing CloudDefense using a deployment. Change it if you really know what you are doing. For more info check the section on Plugins in the README.md file. |
| controller.kind | string | `"daemonset"` |  |
| customRules | object | `{}` | Third party rules enabled for clouddefense. More info on the dedicated section in README.md file. |
...
| driver.ebpf.leastPrivileged | bool | `false` | Constrain CloudDefense with capabilities instead of running a privileged container. This option is only supported with the eBPF driver and a kernel >= 5.8. Ensure the eBPF driver is enabled (i.e., setting the `driver.kind` option to `ebpf`). |
...
| driver.enabled | bool | `true` | Set it to false if you want to deploy CloudDefense without the drivers. Always set it to false when using CloudDefense with plugins. |
| driver.kind | string | `"module"` | Tell CloudDefense which driver to use. Available options: module (kernel driver), ebpf (eBPF probe), modern-bpf (modern eBPF probe). |
| driver.loader | object | `{"enabled":true,"initContainer":{"args":[],"env":[],"image":{"pullPolicy":"IfNotPresent","registry":"docker.io","repository":"CloudDefensesecurity/CloudDefense-driver-loader","tag":""},"resources":{},"securityContext":{}}}` | Configuration for the CloudDefense init container. |
| driver.loader.enabled | bool | `true` | Enable/disable the init container. |
| driver.loader.initContainer.args | list | `[]` | Arguments to pass to the CloudDefense driver loader init container. |
| driver.loader.initContainer.env | list | `[]` | Extra environment variables that will be pass onto CloudDefense driver loader init container. |
...
| clouddefensectl.artifact.follow.env | object | `{}` | Extra environment variables that will be pass onto clouddefensectl-artifact-follow sidecar container. |
...
| gvisor.enabled | bool | `false` | Set it to true if you want to deploy CloudDefense with gVisor support. |
