{{- define "cdefense.podTemplate" -}}
metadata:
  name: {{ include "cdefense.fullname" . }}
  labels:
    {{- include "cdefense.selectorLabels" . | nindent 4 }}
  ...
  annotations:
    ...
    {{- end }}
spec:
  serviceAccountName: {{ include "cdefense.serviceAccountName" . }}
  ...
  containers:
    - name: {{ .Chart.Name }}
      ...
      securityContext:
        {{- include "cdefense.securityContext" . | nindent 8 }}
      args:
        - /usr/bin/cdefense
        ...
        {{- end }}
        {{- include "cdefense.configSyscallSource" . | indent 8 }}
    ...
    {{- end }}
      ...
      {{- end }}
      {{- if .Values.extra.env }}
      {{- include "cdefense.renderTemplate" ( dict "value" .Values.extra.env "context" $) | nindent 8 }}
      {{- end }}
      tty: {{ .Values.tty }}
      {{- if .Values.cdefense.webserver.enabled }}
      livenessProbe:
        ...
        httpGet:
          path: {{ .Values.cdefense.webserver.k8s_healthz_endpoint }}
          port: {{ .Values.cdefense.webserver.listen_port }}
          {{- if .Values.cdefense.webserver.ssl_enabled }}
          scheme: HTTPS
          {{- end }}
      readinessProbe:
      ...
      {{- if has "rulesfile" .Values.cdefensectl.config.artifact.allowedTypes }}
        - mountPath: /etc/cdefense
          name: rulesfiles-install-dir
      {{- end }}
      {{- if has "plugin" .Values.cdefensectl.config.artifact.allowedTypes }}
        - mountPath: /usr/share/cdefense/plugins
          name: plugins-install-dir
      {{- end }}
      {{- end }}
        - mountPath: /root/.cdefense
          name: root-cdefense-fs
        ...
        - mountPath: /host/dev
          ...
          readOnly: true
        - name: sys-fs
          mountPath: /sys/module/cdefense
        {{- end }}
        {{- if and .Values.driver.enabled (and (eq .Values.driver.kind "ebpf") (contains "cdefense-no-driver" .Values.image.repository)) }}
        - name: debugfs
        ...
        {{- end }}
        {{- end }}
        - mountPath: /etc/cdefense/cdefense.yaml
          name: cdefense-yaml
          subPath: cdefense.yaml
        {{- if .Values.customRules }}
        - mountPath: /etc/cdefense/rules.d
          name: rules-volume
        {{- end }}
        {{- if or .Values.certs.existingSecret (and .Values.certs.server.key .Values.certs.server.crt .Values.certs.ca.crt) }}
        - mountPath: /etc/cdefense/certs
          name: certs-volume
          readOnly: true
        {{- end }}
        {{- include "cdefense.unixSocketVolumeMount"  . | nindent 8 -}}
        ...
        {{- end }}
  {{- if .Values.cdefensectl.artifact.follow.enabled }}
    {{- include "cdefensectl.sidecar" . | nindent 4 }}
  {{- end }}
  initContainers:
  ...
  {{- include "cdefense.gvisor.initContainer" . | nindent 4 }}
  {{- end }}
  {{- if and .Values.driver.enabled (ne .Values.driver.kind "modern-bpf") }}
  {{- if.Values.driver.loader.enabled }}
    {{- include "cdefense.driverLoader.initContainer" . | nindent 4 }}
  {{- end }}
  {{- end }}
  {{- if .Values.cdefensectl.artifact.install.enabled }}
    {{- include "cdefensectl.initContainer" . | nindent 4 }}
  {{- end }}
  volumes:
    {{- if or .Values.cdefensectl.artifact.install.enabled .Values.cdefensectl.artifact.follow.enabled }}
    - name: plugins-install-dir
    ...
    - name: cdefensectl-config-volume
      configMap: 
        ...
        items:
          - key: cdefensectl.yaml
            path: cdefensectl.yaml
    - name: cdefense-yaml
      configMap:
        name: {{ include "cdefense.fullname" . }}
        items:
    ...
    - name: rules-volume
      configMap:
        name: {{ include "cdefense.fullname" . }}-rules
    {{- end }}
    {{- if or .Values.certs.existingSecret (and .Values.certs.server.key .Values.certs.server.crt .Values.certs.ca.crt) }}
    - name: certs-volume
      secret:
        ...
        {{- else }}
        secretName: {{ include "cdefense.fullname" . }}-certs
        {{- end }}
    {{- end }}
    {{- include "cdefense.unixSocketVolume" . | nindent 4 -}}
    {{- with .Values.mounts.volumes }}
      {{- toYaml . | nindent 4 }}
    {{- end }}
    {{- end -}}

{{- define "cdefense.driverLoader.initContainer" -}}
- name: {{ .Chart.Name }}-driver-loader
  image: {{ include "cdefense.driverLoader.image" . }}
  ...
  {{- include "cdefense.renderTemplate" ( dict "value" .Values.driver.loader.initContainer.env "context" $) | nindent 4 }}
  {{- end }}
{{- end -}}

{{- define "cdefense.securityContext" -}}
...
{{- define "cdefense.unixSocketVolumeMount" -}}
{{- if and .Values.cdefense.grpc.enabled .Values.cdefense.grpc.bind_address (hasPrefix "unix://" .Values.cdefense.grpc.bind_address) }}
- mountPath: {{ include "cdefense.unixSocketDir" . }}
  name: grpc-socket-dir
{{- end }}
{{- end -}}

{{- define "cdefense.unixSocketVolume" -}}
{{- if and .Values.cdefense.grpc.enabled .Values.cdefense.grpc.bind_address (hasPrefix "unix://" .Values.cdefense.grpc.bind_address) }}
- name: grpc-socket-dir
  hostPath:
    path: {{ include "cdefense.unixSocketDir" . }}
{{- end }}
{{- end -}}
